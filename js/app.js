    ...
    (() => {
        ...
        function menuInit() {
            if (document.querySelector(".icon-menu")) document.addEventListener("click", (function(e) {
                if (bodyLockStatus && e.target.closest(".icon-menu")) {
                    ...
                    if (document.documentElement.classList.contains("_catalog-open")) document.documentElement.classList.remove("_catalog-open");
                    if (document.querySelector(".sub-menu-bottom-header__block").classList.contains("_sub-menu-open")) document.querySelector(".sub-menu-bottom-header__block").classList.remove("_sub-menu-open");
                }
            }));
        }
        function menuClose() {
        ...
        function initSliders() {
            if (document.querySelector(".main-block__slider")) new core(".main-block__slider", {
                ...
                pagination: {
                    el: ".controlls-main-block__dots",
                    clickable: true
                },
                on: {
                    init: function(swiper) {
                        console.log(swiper);
                        const allSlides = document.querySelector(".fraction-controlls__all");
                        const allSlidesItems = document.querySelectorAll(".slide-main-block:not(.swiper-slide-duplicate)");
                        allSlides.innerHTML = allSlidesItems.length < 10 ? `0${allSlidesItems.length}` : allSlidesItems.length;
                    },
                    slideChange: function(swiper) {
                        const currentSlide = document.querySelector(".fraction-controlls__current");
        ...
        function documentActions(e) {
            const targetElement = e.target;
            const blockMenu = document.querySelectorAll(".sub-menu-bottom-header__block");
            if (blockMenu.length) blockMenu.forEach((item => {
                const menuItems = item.querySelectorAll(".sub-menu-bottom-header__list").length;
                item.classList.add(`sub-menu-bottom-header__block_${menuItems}`);
            }));
            ...
            if (targetElement.closest(".menu-bottom-header__back")) {
                document.documentElement.classList.remove("_catalog-open");
                e.preventDefault();
            }
            if (targetElement.closest(".sub-menu-bottom-header__back")) {
                const subMenuBlocks = document.querySelectorAll(".sub-menu-bottom-header__block");
                const subMenuLinks = document.querySelectorAll(".menu-bottom-header__link");
                subMenuBlocks.forEach((Block => {
                    Block.classList.remove("_sub-menu-open");
                    e.preventDefault();
                }));
                subMenuLinks.forEach((Link => {
                    Link.classList.remove("_sub-menu-active");
                }));
            }
        }
        window["FLS"] = true;
